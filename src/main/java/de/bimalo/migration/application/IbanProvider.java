...
final class IbanProvider {
  ...
  public String findIban(@NonNull String givenName, @NonNull String lastName) {
    ...
    DirectDebitTransactionInformation9 debitor =
        doc.getCstmrDrctDbtInitn().getPmtInf().get(0).getDrctDbtTxInf().stream()
            .filter(
                d ->
                    String.format("%s, %s", lastName, givenName)
                        .equalsIgnoreCase(d.getDbtr().getNm()))
            .findAny()
            .orElse(null);
    if (debitor != null) {
  ...
  private static Document unmarshal(InputStream inStream) {
    try {
      ...
      JAXBElement<Document> root =
          jaxbUnmarshaller.unmarshal(new StreamSource(inStream), Document.class);
